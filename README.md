# Hi there, I'm Poorvith Devang 👋

<p align="left">
  <img src="https://img.shields.io/badge/Location-Bengaluru-16a34a?style=flat-square" alt="Location: Bengaluru" />
  <img src="https://img.shields.io/badge/Open%20to-Work-f59e0b?style=flat-square" alt="Open to work" />
</p>

## About Me

- CS + Design background, focused on building polished product experiences.
- Into UI/UX, full-stack development, and practical automation.
- Based in Bengaluru.
- Currently building and shipping projects that are useful and cleanly engineered.

## Connect With Me

- Portfolio: [poorvithdevang861.github.io/poorvith-portfolio](https://poorvithdevang861.github.io/poorvith-portfolio/)
- GitHub: [@poorvithdevang861](https://github.com/poorvithdevang861)

## Featured Projects

- **[bloodbankdb](https://github.com/poorvithdevang861/bloodbankdb)**  
  Flask + Oracle Blood Bank Management System with role-based access and workflow automation.

- **[customshell](https://github.com/poorvithdevang861/customshell)**  
  Modular shell with REPL, built-ins, pipes, redirection, and background jobs.

- **[Automation-Folders](https://github.com/poorvithdevang861/Automation-Folders)**  
  Batch-sort PDF files based on OCR-extracted naming patterns.

- **[Intelligence-dashboard](https://github.com/poorvithdevang861/Intelligence-dashboard)**  
  Interactive dashboard for Indian crime data analytics and filtering.

## Skills & Tools

![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=oracle&logoColor=white)

## Current Focus

- Building production-ready full-stack systems.
- Improving interface quality and developer experience.
- Contributing to projects where engineering quality matters.
